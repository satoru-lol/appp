...
<div class="bread_crumb">
    <div class="container">
        <ul>
            ...
            <li>{{\Illuminate\Support\Facades\Route::is('editClub') ? 'Редактирование курса' : 'Добавление курса'}}</li>
        </ul>
    </div>
</div>
<div class="theme_block second_ht">
    <div class="container">
        <div class="title">
            <h2>{{\Illuminate\Support\Facades\Route::is('editCourse') ? 'Редактирование курса' : 'Добавление курса'}}</h2>
        </div>
<form method='post' action="{{\Illuminate\Support\Facades\Route::is('editCourse') ? route('updateCourse') : route('courses.store')}}" enctype="multipart/form-data">
@csrf
 ...
 @if ($errors->any())
    ...
    @endif
            <div class="name_post">
                ...
                <input type="text" name="times[read]" value="{{$item->times['read'] ?? ""}}" placeholder="Время чтения">
            </div>
            <div class="name_post">
                <input type="text"  name="times[training]" value="{{$item->times['training'] ?? ""}}" placeholder="Срок обучения">
                <input type="text"  name="times[start]" value="{{$item->times['start'] ?? ""}}" placeholder="Когда старт?">
            </div>
			 <div class="name_post">
    ...
    <div id="videoList"></div><br>
            ...
            <textarea name="desc" placeholder="Описание курса">{{!empty($course->text) ? $course->text : ""}}</textarea>
            ...
            <input type="text" name="content[teach3][subtitle]" placeholder="Чему вас научит курс (подзаголовок)">-->
			<input type="text" name="speakers" value="{{!empty($course->speakers) ? $course->speakers : ""}}" placeholder="Спикеры курса">
            ...
            <div style="display: flex">
                <div>
                    ...
                    <label class="btn btn-outline-success" for="pay_for_other">Оплата за курс отдельно <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" fill="currentColor" class="bi bi-cash" viewBox="0 0 16 16">
           ...
           <!-- <div id="fileList"></div>
            ...
            <button>{{\Illuminate\Support\Facades\Route::is('editCourse') ? 'Редактировать курс' : 'Добавить курс'}}</button>
        </form>
