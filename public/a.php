...
<body>
            ...
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div id="input-prompt-container" class="d-none mt-3">
                            <div id="input-prompt" class="mb-2"></div>
                            <form id="input-form" class="d-flex">
                                <input type="text" id="input-answer" class="form-control me-2" placeholder="Введите ответ...">
    ...
    <script>
        $(document).ready(function() {
            ...
            // Сохраняем последние данные запроса
            ...
            $('#command-form').on('submit', function(e) {
                ...
                // Сохраняем для повторного использования при интерактивном вводе
                lastCommandData = {
                    command: command,
                    args: args,
                    options: options
                ...
                $('#output').addClass('d-none');
                $('#input-prompt-container').addClass('d-none');
                // Отправляем AJAX запрос
                sendCommandAjax(lastCommandData);
            });

            // Обработчик отправки интерактивного ответа
            $('#input-form').on('submit', function(e) {
                e.preventDefault();
                const userInput = $('#input-answer').val();
                if (!userInput) return;
                // Добавляем input к последнему запросу
                const data = Object.assign({}, lastCommandData, { input: userInput });
                $('#loading').removeClass('d-none');
                $('#input-prompt-container').addClass('d-none');
                sendCommandAjax(data);
            });

            // Функция отправки AJAX-запроса (обычного и с интерактивным вводом)
            function sendCommandAjax(data) {
                $.ajax({
                    url: '',
                    type: 'POST',
                    dataType: 'json',
                    ...
                    success: function(response) {
                        ...
                        if (response.need_input) {
                            // Показываем prompt и поле для ввода
                            $('#input-prompt').text(response.prompt);
                            $('#input-answer').val('');
                            $('#input-prompt-container').removeClass('d-none');
                            $('#output').removeClass('d-none').html('<span class="text-muted">Ожидание ответа...</span><br>' + (response.output ? '<pre>' + response.output + '</pre>' : ''));
                        } else {
                            // Показываем результат
                            $('#input-prompt-container').addClass('d-none');
                            $('#output').removeClass('d-none');
                            if (response.error) {
                                $('#output').html('<div class="error">' + response.error + '</div>' + (response.output ? '<pre>' + response.output + '</pre>' : ''));
                            } else {
