...
<div class="bread_crumb">
    <div class="container">
        <ul>
            ...
            <li><a href="{{ route('courses') }}">Клубы <span>—</span></a></li>
            <li>{{\Illuminate\Support\Facades\Route::is('editClub') ? 'Редактирование клуба' : 'Добавление клуба'}}</li>
        </ul>
    </div>
</div>
<div class="theme_block second_ht">
    <div class="container">
        <div class="title">
            <h2>{{\Illuminate\Support\Facades\Route::is('editClub') ? 'Редактирование клуба' : 'Добавление клуба'}}</h2>
        </div>
<form method='{{\Illuminate\Support\Facades\Route::is('editClub') ? 'post' : 'post'}}' action="{{\Illuminate\Support\Facades\Route::is('editClub') ? route('editClub', [$club->id]) : route('club.store')}}" enctype="multipart/form-data">
@csrf
 ...
 @if ($errors->any())
    ...
    @endif
            <div class="name_post">
                <input type="text" name="title" value="{{!empty($club->title) ? $club->title : ""}}" placeholder="Название статьи">
                <input type="text" name="times[read]" value="{{!empty($club->times) ? json_decode($club->times, true)["read"] : ""}}" placeholder="Время чтения">
            </div>
            <div class="name_post">
                <input type="text"  name="times[training]" value="{{!empty($club->times) ? json_decode($club->times, true)["training"] : ""}}" placeholder="Срок обучения">
                <input type="datetime-local"  name="date" value="{{!empty($club->date) ? $club->date : ""}}" placeholder="Когда старт?">
            </div>
			 <div class="name_post">
    ...
    <div id="fileList"></div>
        @if(!empty($club->image))
            ...
            <div>
                <img src="/images/{{$club->image}}" name="uploaded_image" style="max-width: 150px;">
                <input type="hidden" name="uploaded_image" value="{{$club->image}}">
            </div>
        @endif
    <br>
            <div class="file-upload-container">
    ...
    <div id="videoList"></div><br>
            ...
            <textarea name="desc" placeholder="Описание клуба">{{!empty($club->text) ? $club->text : ""}}</textarea>
            ...
            <input type="text" name="content[teach3][subtitle]" placeholder="Чему вас научит курс (подзаголовок)">-->
			<input type="text" name="speakers" value="{{!empty($club->speakers) ? $club->speakers : ""}}" placeholder="Спикеры клуба">

            <input type="text" name='theory' value="{{!empty($club->theory) ? $club->theory : ""}}" placeholder="Формат">

            <div style="display: flex">
                <div>
                    <input type="radio" class="btn-check" name="options-outlined" id="pay_for_other" autocomplete="off">
                    <label class="btn btn-outline-success" for="pay_for_other">Оплата за клуб отдельно <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" fill="currentColor" class="bi bi-cash" viewBox="0 0 16 16">
            ...
            <div id="pay_type_form">
                <div id="subscriptions" hidden="hidden">
                    <input type="hidden" id="checked_level" value="{{!empty($club->product_level) ? $club->product_level : ""}}">
                    @foreach ($products as $product)
                        <input type="radio" class="btn-check product-check" @if(!empty($club->product_level) && $club->product_level == $product->level) checked @else "" @endif value="{{ $product->level }}" name="options-outlined" data-row-id="{{ $product->id }}" id="{{ $product->name }}" autocomplete="off">
                ...
                <div id="pay" hidden="hidden">
                    <input type="text" name="practice" id="practice" value="{{!empty($club->practice) ? $club->practice : ""}}" placeholder="Стоимость">
                </div>
            </div><br>
            ...
            <input type="text" name='feedback' value="{{!empty($club->feedback) ? $club->feedback : ""}}" placeholder="Ссылка для кнопки 'Записаться на клуб'">
            <!--<div class="file-upload-container">
                <div class="file-upload-area" onclick="document.getElementById('fileInput').click()">
           ...
           <!-- <div id="fileList"></div>
            ...
            <button>{{\Illuminate\Support\Facades\Route::is('editClub') ? 'Редактировать клуб' : 'Добавить клуб'}}</button>
        </form>
