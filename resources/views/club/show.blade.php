...
<div class="bread_crumb">
    <div class="container">
        <ul>
            ...
            <li><a href="{{ route('v2.club.index') }}">Клуб <span>—</span></a></li>
            <li>{{ $club->title }}</li>
        </ul>
    </div>
</div>
        ...
        <div class="info">
            <div class="left_block_info">
                ...
                <div class="img_block" style="text-align: justify">
                    <img src="/images/{{$club->image}}" width='100%' alt="">
                </div><br>
                <h2>{{ $club->title ?? '' }}</h2>
                <div class="block_top">
                    <div class="work_time">
                        <img src="/img/img_cl.svg" alt="">
                        <p>{{ date('d.m.Y',strtotime($club->clubDates->date)) }}</p>
                    </div>
                    <div class="work_time">
                     ...
                     @php
                            ...
                            @if ($times && isset($times->read))
                                <p>{{ $times->read }}</p>
                            @else
                                <p>Нет данных о времени чтения.</p>
                    @endif
                    </div>
                </div>
                <div class="block_rice">
                    
				<p><b>Формат:</b> {{$club->theory}}</p>
				</div>
					<div class="block_rice">
				<!--<p><b>Стоимость:</b> {{$club->practice != "0" ? $club->practice." руб." : ($product ? $product->name." подписка" : "подписка")}} </p>-->
				</div>
				<div class="block_rice">
				<p><b>Преподаватели:</b> {{$club->clubDates ? $club->clubDates->speakers : 'Не указаны'}}</p>
				</div>
                <div class="block_rice">

                    <div class="join_a">
                        @if ($subscriptionStatus->hasClubAccess)
                            {{-- Если у пользователя есть доступ, показываем ссылку на фидбек (подключение) --}}
                            <a href="{{ $club->feedback }}" target="_blank" class="btn-join-club">Подключиться к клубу</a>
                        @else
                            {{-- Если доступа нет, показываем кнопку, открывающую модальное окно --}}
                            <a href="#" data-bs-toggle="modal" data-bs-target="#subscriptionModal" class="btn-join-club-disabled">Вступить в клуб</a>
                        @endif
                    </div>
                        ...
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    Для вступления в клуб нужно повысить уровень подписки. Уровень подписки можно повысить в <a href="{{ route('v2.profile.index') }}">личном кабинете</a>.
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Закрыть</button>
                                    <a href="{{ route('v2.profile.index') }}" class="btn btn-primary">Перейти в личный кабинет</a>
                                </div>
                            </div>
                        </div>
                    </div>
 ...
 {{--       <style>
        ...
        @if(!empty($club->video))
            <video width="640" height="360" controls>
                <source src="https://appp-psy.ru/images/{{$club->video}}" type="video/mp4">
