...
class UserPresenter extends Presenter implements Personable, Searchable
{
    ...
    {
        return $this->entity->name ?? $this->entity->firstname ?? 'Ошибка в имени юзера';
    }
    ...
    public function image(): ?string
    {
        ...
        if (empty($email)) {
            $introduction = Introduction::where('phone', $this->entity->phone)->first();
            $email = $introduction ? $introduction->email : null;
        }
        ...
        if (empty($this->entity->email)) {
            $this->entity->email = $introduction->email;
            $this->entity->save();
        }

        $hash = md5(strtolower(trim($email)));
        $defaultAvatarUrl = urlencode('https://raw.githubusercontent.com/orchidsoftware/.github/main/web/avatars/gravatar.png');

        return "https://www.gravatar.com/avatar/$hash?d=$defaultAvatarUrl";
    }
